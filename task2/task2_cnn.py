...
def transform(dataset,dicts,train=True):
    num=dataset.shape[0]
    out=[]
    label=[]
    for i in range(num):
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.block1=nn.Sequential(nn.Conv2d(1,num_filter,(filter_size[0],EMBEDDING_SIZE),bias=True),
                                  nn.Dropout(dropout_rate),
                                  nn.ReLU(),
                                  nn.MaxPool2d((sentence_maxlength-filter_size[0]+1,1)),
                                  )
        self.block2 = nn.Sequential(nn.Conv2d(1, num_filter, (filter_size[1], EMBEDDING_SIZE), bias=True),
                                    nn.Dropout(dropout_rate),
                                    nn.ReLU(),
                                    nn.MaxPool2d((sentence_maxlength - filter_size[1] + 1, 1)),
                                    )
        self.block3 = nn.Sequential(nn.Conv2d(1, num_filter, (filter_size[2], EMBEDDING_SIZE), bias=True),
                                    nn.Dropout(dropout_rate),
                                    nn.ReLU(),
                                    nn.MaxPool2d((sentence_maxlength - filter_size[2] + 1, 1)),
                                    )
        self.block4 = nn.Sequential(nn.Conv2d(1, num_filter, (filter_size[3], EMBEDDING_SIZE), bias=True),
                                    nn.Dropout(dropout_rate),
                                    nn.ReLU(),
...
glove_vocab,glove_emb,dic=read_pretrain('glove-6b-100d/glove_6B_100d.txt')
...
train_emb,train_label=transform(train_data,dicts,train=True)
test_emb,_=transform(test_data,dicts,train=False)
